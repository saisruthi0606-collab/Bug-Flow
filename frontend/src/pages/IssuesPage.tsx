    ...
    <Layout title="Issues">
      ...
      {impactIssue && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" role="dialog" aria-modal="true" aria-labelledby="impact-title">
        <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-border bg-card p-6 shadow-glow">
          ...
          {impactLoading && <p className="mt-6 text-sm text-muted-foreground">Preparing impact analysis...</p>}
          {impactError && <p className="mt-6 text-sm text-rose-400">{impactError}</p>}
          {impact && <div className="mt-5 grid gap-4 text-sm"><div className="flex flex-wrap gap-3"><span className="rounded-full bg-primary/10 px-3 py-1">Impact Level: <strong>{impact.impact_level}</strong></span><span className="rounded-full bg-accent/10 px-3 py-1">Regression Risk: <strong>{impact.regression_risk}</strong></span>{impact.confidence !== null && <span className="rounded-full bg-background px-3 py-1">Confidence: <strong>{impact.confidence}%</strong></span>}</div>{impact.status === 'pending' && <p className="text-muted-foreground">Generating enhanced AI analysis...</p>}<p><strong>Affected Areas</strong><br/>{impact.affected_areas.join(' • ')}</p>{[['Expected Effects',impact.expected_effects],['Possible Side Effects',impact.possible_side_effects],['Recommended Testing',impact.recommended_testing],['Recommended Precautions',impact.recommended_precautions]].map(([label,items])=><div key={String(label)}><strong>{label}</strong><ul className="mt-1 list-disc space-y-1 pl-5">{(items as string[]).map(item=><li key={item}>{item}</li>)}</ul></div>)}<p><strong>AI Summary</strong><br/>{impact.summary}</p>{impact.status === 'unavailable' && <p className="text-amber-500">AI-enhanced explanation is currently unavailable.</p>}</div>}
        </div>
      </div>}
    </Layout>
  )
